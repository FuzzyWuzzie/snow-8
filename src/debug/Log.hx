...
class Log {
	...
	public static function log(v:Dynamic, func:LogFunctions, ?pos:haxe.PosInfos):Void {
			...
			switch(func) {
				case Fatal: {
					untyped console.log('%cf.' + pos.fileName + ':' + pos.lineNumber + ': ' + message, 'font-weight: bold; color: white; background: #A84B38;');
	...
	macro public static function fatal(value:Dynamic):Expr {
		#if log_fatal
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Fatal);
		#else
			return macro null;
		#end
	}
	...
	macro public static function error(value:Dynamic):Expr {
		#if log_error
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Error);
		#else
			return macro null;
		#end
	}
	...
	macro public static function warning(value:Dynamic):Expr {
		#if log_warning
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Warn);
		#else
			return macro null;
		#end
	}
	...
	macro public static function info(value:Dynamic):Expr {
		#if log_info
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Info);
		#else
			return macro null;
		#end
	}
	...
	macro public static function debug(value:Dynamic):Expr {
		#if debug
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Debug);
		#else
			return macro null;
		#end
	}
	...
	macro public static function trace(value:Dynamic):Expr {
		#if log_trace
			return macro @:pos(Context.currentPos()) debug.Log.log($value, debug.Log.LogFunctions.Trace);
		#else
			return macro null;
		#end
	}
